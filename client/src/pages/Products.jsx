...
const Products = () => {
  ...
  // State for functional image slider dots
  const [activeImageIndices, setActiveImageIndices] = useState({});
  
  // Validation State for warnings
  ...
  const handleSaveOrUpdate = async (e) => {
    e.preventDefault();
    if (!validateForm()) return; // Trigger warnings if fields empty
  ...
  return (
    <div className="flex-1 flex flex-col h-screen overflow-hidden">
      ...
      <header className="bg-white shadow-sm flex-shrink-0">
        ...
        <div className="h-14 flex items-center justify-between px-8 border-t border-slate-50">
          <div className="flex items-center text-[18px] font-bold text-[#445069]">Products</div>
          <button onClick={openAddModal} className="flex items-center gap-2 text-[#445069] font-medium text-[20px] hover:text-[#1D35D9] transition-colors">
              <span className="text-[24px] font-light text-[#94a3b8]">+</span> Add Products
          </button>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto relative bg-[#F8F9FA]">
        {products.length === 0 ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4 bg-white">
            <div className="mb-6">
              <svg width="100" height="100" viewBox="0 0 24 24" fill="none" className="text-[#001D9D] mx-auto opacity-20">
                <path d="M3 3H10V10H3V3ZM3 14H10V21H3V14ZM14 14H21V21H14V14ZM17 3V7M15 5H19" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </div>
                ...
                <div key={p._id} className="bg-white rounded-[1.5rem] shadow-sm border border-slate-100 overflow-hidden flex flex-col w-full hover:shadow-lg transition-shadow">
                  ...
                  <div className="p-6 pt-0 flex flex-col flex-1">
                    ...
                    <div className="flex gap-2 mt-6">
                      <button onClick={() => togglePublishStatus(p)} className={`flex-1 py-2 rounded-lg font-bold text-xs text-white shadow-sm transition-all active:scale-95 ${p.status === 'Published' ? 'bg-[#1D35D9]' : 'bg-[#4CD100]'}`}>{p.status === 'Published' ? 'Unpublish' : 'Publish'}</button>
                      ...
                      <button onClick={() => openDeleteModal(p)} className="w-10 h-10 border border-slate-200 rounded-lg flex items-center justify-center text-slate-400 hover:text-red-500">🗑️</button>
                    </div>
                  </div>
                </div>
              );
      ...
      {/* ADD/EDIT MODAL WITH VALIDATION WARNINGS */}
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-[1px] flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-lg w-full max-w-[420px] shadow-2xl flex flex-col max-h-[95vh] overflow-hidden">
            ...
            <form onSubmit={handleSaveOrUpdate} className="p-6 space-y-4 overflow-y-auto">
              {/* Product Name Field with Warning */}
